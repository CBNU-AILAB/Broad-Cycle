...
class Babroad:
    ...
    def train(self, config: Configuration, seed: int = 0) -> float:
        with warnings.catch_warnings():
            ...
            global tetime_list, total_time_list, time_count, input_list, stage_list, cell_list, cell_dim_list, parm_list, mac_list, total_mac_list
            input_list.append(infeature_list)
            stage_list.append(stage)
            cell_list.append(cell)
            cell_dim_list.append(start_dim)
            mac_list.append(string_macs)
            total_mac_list.append(macs)
...
if __name__ == "__main__":
    PATH = "/home/sin/PycharmProjects/please/DSN/data/UCR/"
    timepth = "/home/gpuadmin/beom/data/ucr/NonInvasiveFetalECGThorax1/"
    ucr = fetch_ucr_dataset('NonInvasiveFetalECGThorax1', use_cache=True, data_home=PATH, return_X_y=False)  # 5 class
    ucr_train = ucr.data_train  # 390, 176
    ucr_target_train = ucr.target_train
    ucr_test = ucr.data_test
    ucr_target_test = ucr.target_test
    ucr_train_re = resize(ucr_train, ((1800, 550, 1)))
    ucr_test_re = resize(ucr_test, ((1965, 550, 1)))
    ...
    scenario = Scenario(mlp.configspace, n_trials=1000)
    ...
    for i in range(time_count):
        ...
        all_macs = all_macs+total_mac_list[i]
    avg_trtime = all_trtime/time_count
    avg_tetime = all_tetime/time_count
    avg_total_time = all_total_time/time_count
    avg_macs = all_macs/time_count
    print("avg_train_time : ", avg_trtime)
    print("avg_test_time : ", avg_tetime)
    print("avg_all_time : ", avg_total_time)
    print("avg_macs : ", avg_macs)
    print("total_explor_time : ", all_total_time)
    num = len(cell_list)
    df_results = pd.DataFrame(index=range(0, num),
                              columns=['img_coding', 'num_stage', 'num_cell', 'start_cell_dim',
                                       'parameter_num', 'train_time', 'test_time', 'total_time','mac'])
    df_results['img_coding'] = input_list
    df_results['num_stage'] = stage_list
    df_results['num_cell'] =cell_list
    df_results['start_cell_dim'] =cell_dim_list
    df_results['parameter_num'] =parm_list
    df_results['train_time'] =trtime_list
    df_results['test_time'] =tetime_list
    df_results['total_time'] =total_time_list
    df_results['mac'] = mac_list

    df_time = pd.DataFrame(index=range(0, 1),columns=['avg_train_time', 'avg_test_time', 'avg_total_time',
                                                                       'total_explor_time','avg_macs'])
    df_time['avg_train_time'] =avg_trtime
    df_time['avg_test_time'] =avg_tetime
    df_time['avg_total_time'] =avg_total_time
    df_time['total_explor_time'] =all_total_time
    df_time['avg_macs'] = avg_macs
    df_results.to_csv(timepth+"each_time_result.csv")
    df_time.to_csv(timepth + "avg_time_result.csv")

...
class Babroad:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        cs.add_hyperparameters(
            [RP, RERP, TRRP, GAS, REGAS, TRGAS, GAD, REGAD, TRGAD, MK, REMK, TRMK, CTW])
    ...
    def train(self, config: Configuration, seed: int = 0) -> float:
        with warnings.catch_warnings():
            ...
            if img_test_acc < test_acc:
                ...
                save_num = save_num+1
                torch.save(model.state_dict(), PATH +str(save_num) +'NonIncasiveFetalECGThorax1(new).pt')
...
if __name__ == "__main__":
    ucr = fetch_ucr_dataset("NonInvasiveFetalECGThorax1", use_cache=True, data_home=None, return_X_y=False)  # 5 class
    ucr_train = ucr.data_train  # 390, 176
    ucr_target_train = ucr.target_train
    ucr_test = ucr.data_test
    ucr_target_test = ucr.target_test
    ucr_train_re = resize(ucr_train, ((1800, 128, 1)))
    ucr_test_re = resize(ucr_test, ((1965, 128, 1)))
    ...
    PATH = "C:\\Users\\user\\PycharmProjects\\please\\save\\NonInvasiveFetalECGThorax1\\"
    ...
    df_results.to_csv(PATH+"NonInvasiveFetalECGthorax1.csv")
